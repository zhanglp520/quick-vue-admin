...
const noticeMe = () => {
  getOrderStatistics().then((res) => {
    const { payload } = res
    const { dayOrderNum, yesterDayOrderNum, weekOrderNum, monthOrderNum } =
      payload
...
<template>
  <el-row>
    <el-col :span="24">
      ...
      <el-switch v-model="isOpen" active-text="打开提示音" />
    </el-col>
  </el-row>
  <audio id="audio" muted>
    <source src="@/assets/audios/order.mp3" type="audio/mpeg" />
  </audio>
  <el-row>
    <el-col :span="24">
